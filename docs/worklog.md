...
- **vc65 / 1.6 — SHIPPED ✅ 2026-07-10** (run `29110384308` CONFIRMED SUCCESS ~6m13s, published to closed testing; commit `d56ec66`): 3-brief vc64 follow-up
  ...
  topBar so it stays pinned), map-tab flash (brand-teal loading scrim + SplashGoldBright spinner;
  off-screen chevron tracker gated on cameraPositioned). On-device verify waived pre-ship. Detail in
  `worklog-2026-07-10.md`.
...
- **vc62 / 1.6 — SHIPPED ✅ 2026-07-06** (WIF run `28783329685` CONFIRMED SUCCESS ~6m9s, bump commit
  ...
  Help & About; deviation: brief's `offset` import corrected to
  `androidx.compose.foundation.layout.offset`), `profile-adventures-collapsible-header` (Profile collapses
  on scroll, Adventures collapses on first map pan/zoom). All three briefs → `docs/DONE/`. Detail in
  `worklog-2026-07-06.md`.
...
- **vc60 / 1.6 — SHIPPED (✅ confirmed) 2026-07-05 17:37 +02:00** (WIF run `28745924842`, bump commit
  `d113df4`, impl `c19a628`; PUBLISHED to closed testing). `landscape-layout-consistency-v2` brief
  (5 changes, 4 files): Adventures map
